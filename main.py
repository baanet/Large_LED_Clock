...
def side_kick():
    ...
    try:
        ...
        tm1.show(str(int(raw_temperature))+str(weather_data.json().get('main').get('humidity')))
        t1 = (str(int(raw_temperature)))
        if len(t1) == 1:
            t1 = '0'+t1
        h1 = (str(weather_data.json().get('main').get('humidity')))
        if len(h1) != 3:
            h1 = " "+h1
        tm1.write(tm1.encode_string(t1+h1))
        print(str(int(raw_temperature)))
        print(time.localtime())
        tm1.brightness(tm.brightness())

    except:
        print(time.localtime())
        print("No internet at this time")
        tm1.write(tm1.encode_string("     "))
        tm1.brightness(tm.brightness())

def week_day():
    global wd1, np
    tw = ds.get_time()#time.localtime()
    wd = [6,5,4,3,2,1,0]
    wd1 = wd[tw[6]]
    np = neopixel.NeoPixel(Pin(7), 7)
    np.BRIGHTNESS= .2
    np[wd1] = (125, 125, 125) # set to red, full brightness
    np.write()
    time.sleep(5)
...
while True:
    ...
    if ds.get_time()[3] in range(7, 22):
        tm.brightness(2)
        tm1.brightness(1)
        print("Brightness ", tm.brightness())
        np[wd1] = (255, 255, 255)
        np.write()
    else:
        tm.brightness(0)
    ...
    if hr24 == 0:
        tm.numbers(ds.get_time()[3], ds.get_time()[4])#, colon = False)
    else:
        ...
        tm.show(show, colon = True) # send time to the display as text
        #----- alt changes----------
    if sk == 0:
        side_kick()
        sk = 15
    print(60 - ds.get_time()[5]+1,"Seconds to next update")
    #print(t)
    sk = sk-1
